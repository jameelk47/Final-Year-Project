...
class HeteroscedasticKerasRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if self.random_state is not None:
            ...
            # Set TensorFlow's global random seed
        ...
        input_dim = X.shape[1]
        self.model_ = build_hnn_model(input_dim, learning_rate=self.learning_rate, random_state=self.random_state)
...
# Save the Keras model inside the wrapper
hnn.model_.save('hnn_weights.keras')
# Save the wrapper itself (to keep n_samples, etc.)
